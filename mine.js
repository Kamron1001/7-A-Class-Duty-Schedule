document.addEventListener('DOMContentLoaded', function() {
    ...
    // Функция для вычисления следующего времени обновления
    function getNextUpdateTime() {
        const now = new Date();
        const nextUpdate = new Date();
        nextUpdate.setHours(24, 0, 0, 0); // Устанавливаем на полночь
        if (now >= nextUpdate) {
    ...
    // Сохранение текущего состояния (индекс, время следующего обновления)
    ...
    function updateSchedule() {
        ...
        saveState(currentIndex, nextUpdateTime); // Сохраняем текущее состояние
    }
    ...
    function checkUpdate() {
        ...
        if (now >= nextUpdateTime) {
            updateDay(); // Обновляем день, если текущее время больше времени обновления
        } else {
    ...
    // Останавливаем движение временно: чтобы остановить — закомментируй строку ниже
    // setInterval(checkUpdate, 1000);// Эта строка отвечает за движение линии
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Остановка линии: Просто не меняем currentIndex
    function stopLine() {
        // Линия остановлена, currentIndex остаётся без изменений
        updateSchedule(); // Применяем текущее состояние
    }
