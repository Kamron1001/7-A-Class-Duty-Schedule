document.addEventListener('DOMContentLoaded', function () {
    ...
    function moveUp() {
        if (currentIndex === 0) {
            currentIndex = rows.length - 1; // Если текущая строка первая, перемещаем на последнюю
        } else {
            currentIndex--; // Сдвигаем индекс вверх
        }
        updateSchedule(); // Обновляем расписание
    }

    // Перемещение линии вниз
    function moveDown() {
        if (currentIndex === rows.length - 1) {
            currentIndex = 0; // Если текущая строка последняя, перемещаем на первую
        } else {
            currentIndex++; // Сдвигаем индекс вниз
        }
        updateSchedule(); // Обновляем расписание
    }
    ...
    // Добавляем кнопки для перемещения линии вверх и вниз
    const upButton = document.createElement('button');
    upButton.innerText = '⬆ Вверх';
    upButton.style.cssText = `
        ...
        margin-right: 10px;
    `;
    upButton.addEventListener('click', moveUp); // Добавляем событие клика
    document.body.appendChild(upButton); // Добавляем кнопку на страницу

    const downButton = document.createElement('button');
    downButton.innerText = '⬇ Вниз';
    downButton.style.cssText = `
        ...
        transition: background 0.3s ease, color 0.3s ease;
    `;
    downButton.addEventListener('click', moveDown); // Добавляем событие клика
    document.body.appendChild(downButton); // Добавляем кнопку на страницу
