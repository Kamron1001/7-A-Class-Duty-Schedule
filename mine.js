...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Получаем текущий индекс из LocalStorage или устанавливаем в 0, если нет данных
    let currentIndex = parseInt(localStorage.getItem('currentIndex')) || 0;
    
    // Функция для обновления отображения текущей строки
    function updateSchedule() {
        rows.forEach(row => row.classList.remove('highlight')); // Убираем выделение со всех строк
        if (currentIndex >= 0 && currentIndex < rows.length) {
            rows[currentIndex].classList.add('highlight'); // Выделяем текущую строку
        }
        localStorage.setItem('currentIndex', currentIndex); // Сохраняем индекс в LocalStorage
    }

    // Функция для перемещения строки на одну назад
    function moveRowBackward() {
        currentIndex = (currentIndex - 1 + rows.length) % rows.length; // Переход на одну строку назад, учитывая цикличность
        updateSchedule(); // Обновляем отображение
    }

    // Вызов функции для перемещения строки на одну назад
    moveRowBackward();
});
